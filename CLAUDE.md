...
| Codexの参加 | — | **note 下書き保存中・未公開** |

**未着手候補（書きたい順、連続性なし）:**
- 外出先での音声メモ格納（CarPlay → Apple Watch）※塊2-F「音声メモ pipeline」と統合（同じお題のため、2026-05-20 整理）
...
### 公開待ち直近キュー
```
Codexの参加 公開（オーナー作業待ち）→ 公開後対応一気通貫
  ↓
第二の脳 未着手 1 本（音声メモ＝塊2-F 統合）or 塊2 残り（E）着手
  ↓
（本編1 ⑱⑲⑳〜 は 2026-05-20 オーナー判断でしばらく保留）
```

ストック枯渇（Output/ は Codexの参加 1 件のみ・未公開）。本編1 保留中のため、第二の脳「音声メモ格納」と塊2 残り 1 本（E）が直近の弾。eventmap 由来の cross-repo ネタ候補（admin 認証 / Codex 品質チェック / Supabase rate limit）も発掘候補。**塊2-C ダッシュボードは実装待ちのため当面書けない**。

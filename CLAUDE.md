...
| ~~失敗02 候補~~ | ~~仕事を任せていたAIが手順を省略していた（仮）~~ | **第二の脳シリーズに転用 → 「magiの手抜き事件」として 2026-05-11 公開済**（本編2 カテゴリでは欠番） |
| ~~失敗03 候補~~ | ~~見落とし（special-10 由来）~~ | **ボツ（2026-05-20 オーナー判断: エピソードとして薄い）** |
...
連続性なし・カテゴリ運用方針（章番号は付けない、書きたい順に投下）。公開順:

| 主題 | 公開日 | 備考 |
|---|---|---|
| 今の外部脳の仕組み | 2026-05-01 | プロローグ表記 |
| なぜobsidianを選んだのか | 2026-05-08 | |
| Magiについて | 2026-05-09 | |
| magiの手抜き事件 | 2026-05-11 | 旧本編2「失敗02」転用、Magi 手抜き連鎖 |
| 日々作られるファイルを自動で整理 | 2026-05-15 | chat-md ウォッチャー |
| セッションを跨いだ記憶のリレー | 2026-05-15 | リポ内 + リポ間 handoff 両方カバー |
| なぜ情報共有にこだわるのか | 2026-05-15 | 軸の言語化 |
| 担当制リポジトリ | 2026-05-19 | META 構築・3 リポ間情報共有 |
| Codexの参加 | 2026-05-25 | Codex 64GB 事件後の継続採用と役割確定 |
| 閃きを捕まえる | 2026-05-25 | 旧「音声メモ pipeline」塊2-F 統合、Apple Watch + 自動振り分け |

**未着手候補（書きたい順、連続性なし）:**
- （現時点で確定構想なし。次の弾はオーナー判断待ち）

**ボツ・吸収済（2026-05-20 整理）:**
- 他AIエクスポート + Twitter 8GB アーカイブ — ボツ（特殊すぎる）
- 各リポ内セッション間 handoff → 「セッションを跨いだ記憶のリレー」に吸収
- 情報相互保持の体験価値 → 「なぜ情報共有にこだわるのか」に吸収
- 2 リポ間 handoff の仕組み → 「セッションを跨いだ記憶のリレー」「担当制リポジトリ」に吸収
- META 構築と 3 リポ間情報共有 → 「担当制リポジトリ」に吸収
- 各 AI のメモリ仕組み → 「Codexの参加」の「Codexのメモリ仕様」H2 に吸収
- 3AI / 4AI 合議モデル — ボツ（2026-05-20 オーナー判断）
- MyAI 時代の手作り橋（クロージング・時代論） — ボツ（2026-05-20 オーナー判断）
...
### 公開待ち直近キュー
```
Codexの参加 / 閃きを捕まえる 2 本連続公開（2026-05-25）+ calibrator 完了
  ↓
次の弾はオーナー構想化待ち（塊2-E 設計シンプル運用複雑 / cross-repo ネタなど候補）
  ↓
（本編1 ⑱⑲⑳〜 は 2026-05-20 オーナー判断でしばらく保留）
```

**ストック完全枯渇**（Output/ 空、Drafts/ は本編⑱⑲のみ・保留中）。本編1 保留中のため、第二の脳の次ネタ着手 or 塊2-E が直近候補。eventmap 由来の cross-repo ネタ候補（admin 認証 / Codex 品質チェック / Supabase rate limit）も発掘候補。**塊2-C ダッシュボードは実装待ちのため当面書けない**。

### 構成案（仮番）
`knowledge-base/note/Ideas/series-plan.md` に詳細あり（**4塊フレームワーク + 第二の脳詳細は未追記、要メンテ**）。(18)以降は仮番で、途中分割あり得る。最終回は決めない。

### 第二の脳シリーズ詳細
- **構成方針**: 連続性なし・カテゴリ運用（章番号は付けない、書きたい順に投下）/ プロローグだけ「今の外部脳の仕組み」表記で先行公開済
- **ペース**: 週1〜2本（毎日公開はしない）
- **オーナー方針**: シリーズ名は公開直前まで仮で OK / 項番（章番号・ギリシャ文字含む）は付けない / 各章は密度優先 / 微修正前提
- **塊2（外出し小ネタ候補・2026-05-20 整理後）**: E 設計シンプル運用複雑（独立単発、シリーズ外）。**C ダッシュボードは実装待ち**（一次情報なし、実装後ネタ化）。~~A 見落とし~~（エピソード薄い）・~~B 掟~~・~~D 引越し大工事~~ はボツ判定、~~F 音声メモ~~ は第二の脳「音声メモ格納」に統合
...
- 第二の脳プロローグ 2026-05-01 公開（プロローグ v1 崩壊 → 失敗02 ネタ転用 → v2 再生成 → 公開、別途 handoff 参照）
- 第二の脳「magiの手抜き事件」2026-05-11 公開（旧本編2「失敗02 候補」を転用）
- 第二の脳 4 本（自動格納 / 記憶のリレー / 情報共有 / 担当制リポジトリ）2026-05-15〜19 連続公開
- 「Codexの参加」note 下書き保存中・未公開（2026-05-20 時点）

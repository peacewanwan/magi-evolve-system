...
| 項目 | 必須/推奨 | 備考 |
|------|---------|------|
| **Claude Code** | 必須 | Claude Max（Opus利用のため。Sonnetだけなら通常プランでも可） |
| **Obsidian** | 推奨（任意） | vault検索に `obsidian` CLI を使う。なくても動く |
| **defuddle** | 推奨（任意） | Web情報のクリーン取得。`npm install -g defuddle` |

### コスト目安

- 1記事あたり約 **480Kトークン**（enricher〜publisher の全ステージ合計）
- 月4-5本で約 **2M〜2.4Mトークン**
- MES（MAGI EVOLVE SYSTEM）を使う場合は +200K〜300K/記事
- Claude Max プラン推奨（Opus を writer/editor/voice_checker で使用するため）
...
**使いどころ:** 切り口に迷うとき、構想の角度を広げたいとき、品質を最大限上げたいとき。通常パイプラインより +200K〜300Kトークン。
...
### Q: コストを下げるには？
A: writer/voice_checker/editor のモデルを Sonnet に変更できます（CLAUDE.md内で指定）。品質は下がりますが、トークン消費は 1/3〜1/5 になります。
